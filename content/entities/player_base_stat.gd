...
const BASE_ARTIFACT_SNAKE_TOOTH := preload("res://content/artifacts/definitions/snake_tooth.tres")
...
@export_range(0, 999999, 1) var starting_coins := 1000
...
func _build_global_map_faces() -> Array[DiceFaceDefinition]:
	return [
		_build_face("swords", GLOBAL_MAP_MOB_ICON),
		_build_face("swords", GLOBAL_MAP_MOB_ICON),
		_build_face("swords", GLOBAL_MAP_MOB_ICON),
		_build_face("swords", GLOBAL_MAP_MOB_ICON),
		_build_face("swords", GLOBAL_MAP_MOB_ICON),
		_build_face("question_mark", GLOBAL_MAP_EVENT_ICON),
	]
...
func _build_default_artifacts_base() -> Array[ArtifactDefinition]:
	return [BASE_ARTIFACT_SNAKE_TOOTH]
