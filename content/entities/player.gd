...
func reset_for_run() -> void:
	...
	runtime_reward_cubes = [base_stat.get_resolved_base_reward_cube().duplicate(true)]
	runtime_event_cubes.clear()
	runtime_reward_cube = runtime_reward_cubes[0]
...
func get_active_reward_cubes() -> Array[DiceDefinition]:
	if not runtime_reward_cubes.is_empty():
		return runtime_reward_cubes
	if runtime_reward_cube != null:
		return [runtime_reward_cube]
	return []
...
func grant_runtime_cube(cube_definition: DiceDefinition) -> void:
	...
	match runtime_cube.scope:
		...
		DiceDefinition.Scope.EVENT:
			runtime_event_cubes.append(runtime_cube)
		DiceDefinition.Scope.MONEY:
			runtime_money_cube = runtime_cube
