	...
	match face_definition.content_type:
		DiceFaceDefinition.ContentType.ICON:
			if face_definition.icon != null:
				...
				var icon_size := Vector2(face_definition.icon.get_width(), face_definition.icon.get_height())
				var max_icon_side := max(icon_size.x, icon_size.y)
				_icon_sprite.pixel_size = max(min_size / max(max_icon_side, 1.0), 0.0001)
