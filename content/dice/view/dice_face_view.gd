	...
	match face_definition.content_type:
		DiceFaceDefinition.ContentType.ICON:
			if face_definition.icon != null:
				...
				var texture_max_dimension := float(max(face_definition.icon.get_width(), face_definition.icon.get_height()))
				_icon_sprite.pixel_size = max(min_size / max(texture_max_dimension, 1.0), 0.0001)
