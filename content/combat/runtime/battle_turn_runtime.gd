...
static func start_battle(battle_room) -> Dictionary:
	...
	_log_turn_debug("бой начат: ход 1 · %s" % _format_turn_owner(battle_room))
	return get_current_turn_context(battle_room)
...
static func advance_turn(battle_room) -> Dictionary:
	...
	if battle_room.battle_status != &"active":
		return get_current_turn_context(battle_room)
	_log_turn_debug("конец хода: %s" % _format_turn_owner(battle_room))
	...
	if battle_room.current_turn_owner == &"player":
		...
		_mark_turn_start_pending(battle_room)
		_log_turn_debug("начало следующего хода: %s" % _format_turn_owner(battle_room))
		return get_current_turn_context(battle_room)
...
static func process_turn_start_if_pending(battle_room) -> bool:
	if battle_room == null or not bool(battle_room.turn_start_pending):
		return false
	battle_room.turn_start_pending = false
	_log_turn_debug("начало хода: %s" % _format_turn_owner(battle_room))
...
static func _format_turn_owner(battle_room) -> String:
	if battle_room == null:
		return "неизвестно"
	if battle_room.current_turn_owner == &"player":
		return "юнит (ход %d)" % int(battle_room.turn_counter)
	if battle_room.current_turn_owner == &"monster":
		return "монстр#%d (ход %d)" % [int(battle_room.current_monster_turn_index) + 1, int(battle_room.turn_counter)]
	return "none (ход %d)" % int(battle_room.turn_counter)


static func _log_turn_debug(message: String) -> void:
	if not OS.is_debug_build():
		return
	print("[BattleTurn] %s" % message)
