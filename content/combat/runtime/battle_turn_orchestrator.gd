...
func throw_current_turn_dice(context: Dictionary) -> void:
	...
	_bind_player_coin_receiver_metadata(battle_room_data, requests)
...
func _bind_player_coin_receiver_metadata(battle_room_data: BattleRoom, requests: Array[DiceThrowRequest]) -> void:
	...
	for request in requests:
		if request == null:
			continue
		request.metadata["coin_receiver"] = player
