...
func throw_current_turn_dice(context: Dictionary) -> void:
	...
	if battle_room_data.is_player_turn() and battle_room_data.player_instance != null:
		...
		var monster_dice_loadout: Array[DiceDefinition] = monster_view.dice_loadout
		if not monster_dice_loadout.is_empty():
			for dice_definition in monster_dice_loadout:
				if dice_definition == null:
					continue
