...
func resolve_target_descriptor_at_screen_point(owner: Node, ability: AbilityDefinition, screen_point: Vector2, monster_sprite_states: Array[Dictionary]) -> Dictionary:
	if owner.battle_room_data == null or ability == null or ability.target_rule == null:
		return {}

	match ability.target_rule.get_target_hint():
		&"self":
			if screen_point_hits_mesh(owner, owner._player_sprite, screen_point) and owner.battle_room_data.can_target_player():
				return {
					"kind": &"player",
				}
		&"single_enemy":
			for index in range(monster_sprite_states.size() - 1, -1, -1):
				...
				if screen_point_hits_mesh(owner, sprite, screen_point) and owner.battle_room_data.can_target_monster(monster_index):
					return {
						"kind": &"monster",
						"index": monster_index,
					}
		&"all_enemies":
			for monster_state in monster_sprite_states:
				var sprite := monster_state.get("sprite") as MeshInstance3D
				if screen_point_hits_mesh(owner, sprite, screen_point):
					return {
						"kind": &"all_monsters",
					}
			if screen_point_hits_mesh(owner, owner._floor, screen_point):
				return {
					"kind": &"all_monsters",
				}
		&"global":
...
func resolve_activation_target_origin(owner: Node, target_descriptor: Dictionary, base_origin: Vector3, monster_sprite_states: Array[Dictionary]) -> Vector3:
...
func project_mouse_to_horizontal_plane(owner: Node, camera: Camera3D, plane_y: float) -> Vector3:
...
func screen_point_hits_mesh(owner: Node, camera: Camera3D, mesh_instance: MeshInstance3D, screen_point: Vector2) -> bool:
	if mesh_instance == null or not is_instance_valid(mesh_instance) or not mesh_instance.visible:
		return false
	if mesh_instance.mesh == null:
		return false
	var projected_rect := project_mesh_screen_rect(camera, mesh_instance)
	return projected_rect.size.x > 0.0 and projected_rect.size.y > 0.0 and projected_rect.has_point(screen_point)


func has_player_dice_at_screen_point(owner: Node, camera: Camera3D, screen_point: Vector2) -> bool:
	if camera == null or owner.get_world_3d() == null:
		return false
...
func project_mesh_screen_rect(camera: Camera3D, mesh_instance: MeshInstance3D) -> Rect2:
