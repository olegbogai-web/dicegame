...
func _spawn_dash_path(path_points: Array[Vector3]) -> void:
	...
	var template_basis_scale := Vector3(
		template.global_transform.basis.x.length(),
		template.global_transform.basis.y.length(),
		template.global_transform.basis.z.length()
	)
	for index in range(1, path_points.size() - 1):
		...
		dash.global_transform.basis = Basis.from_euler(Vector3(0.0, yaw, 0.0)).scaled(template_basis_scale)
