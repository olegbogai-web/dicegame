...
func _restore_persisted_state() -> void:
	...
	if saved_dash_paths is Array and not (saved_dash_paths as Array).is_empty():
		_restore_dynamic_marker_paths(saved_dash_paths as Array)
	else:
...
func _persist_current_state() -> void:
	...
	GlobalMapRuntimeState.save_snapshot({
		...
		"dash_paths": _export_dynamic_path_state(),
	})
...
func _restore_dynamic_marker_paths(saved_dash_paths: Array) -> void:
	_clear_dynamic_marker_paths()
	for dash_data in saved_dash_paths:
		...
		if not position is Vector3:
			continue
		var rotation_y := float((dash_data as Dictionary).get("rotation_y", 0.0))
		_spawn_dynamic_dash(position as Vector3, rotation_y)


func _export_dynamic_path_state() -> Array[Dictionary]:
...
func _spawn_dash_path(path_points: Array[Vector3]) -> void:
	...
	for index in range(1, path_points.size() - 1):
		...
		_spawn_dynamic_dash(jittered_position, yaw)


func _spawn_dynamic_dash(position: Vector3, yaw: float) -> void:
	if _owner == null:
		return
	if _path_dash_template == null or not is_instance_valid(_path_dash_template):
		return
	...
	if dash == null:
		return
	dash.name = "dynamic_dash"
	dash.visible = true
	dash.global_position = Vector3(position.x, DASH_PATH_SPAWN_Y, position.z)
