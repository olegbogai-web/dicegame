...
func _process_monster_turn_with_ai() -> void:
	if battle_room_data == null or not is_inside_tree() or not battle_room_data.is_monster_turn() or battle_room_data.is_battle_over():
		...
		return
	if _monster_turn_in_progress or _activation_in_progress or _turn_transition_in_progress:
		return
	...
	if action == null or action.is_end_turn():
		...
		call_deferred("_finish_monster_turn")
		return

	_monster_turn_in_progress = true
	call_deferred("_perform_monster_action", action)


func _finish_monster_turn() -> void:
	if battle_room_data != null and battle_room_data.is_monster_turn() and not battle_room_data.is_battle_over():
		_advance_to_next_turn()
	_monster_turn_in_progress = false


func _perform_monster_action(action: MonsterAiAction) -> void:
