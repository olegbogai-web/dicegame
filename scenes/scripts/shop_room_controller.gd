...
func _unhandled_input(event: InputEvent) -> void:
	...
	if (_selection_mode == "upgrade" or _selection_mode == "remove") and picked_offer.is_empty():
		return
...
func _render_modal_cards(entries: Array[Dictionary]) -> void:
	...
	for index in range(entries.size()):
		...
		add_child(card)
		card.visible = true
		card.transform.origin = Vector3(MODAL_CENTER_X + offsets[index], MODAL_ROW_Y, MODAL_CENTER_Z)
