...
Актуализация для дизайна способностей: добавлены состояния `Уклонение` (`evasion`) и `Метка` (`mark`) как противоположная пара. При проектировании новых способностей учитывать, что `Уклонение` расходует 1 стак на полное игнорирование одной вражеской атаки, а `Метка` расходует 1 стак на удвоение входящего урона и входящих негативных статусов от одной вражеской атаки.
...
- `protection_spell`-линейка / «Заклинание защиты» (`content/abilities/definitions/protection_spell*.tres`):
  - owner scope: игрок (`owner_scope = PLAYER`), редкость: `COMMON`;
  - base: требует 1 куб со значением `3` или `5`, накладывает `armor` (`4`) на владельца;
  - ветка `1.x`: усиливает броню (`+6 -> +9`), альтернативная `1.2` меняет профиль на `armor +3` + `fortitude 1`;
  - ветка `2.x`: меняет dice-требование на нечетный куб, вариант `2.1` смягчает до любого куба, вариант `2.2` усиливает броню до `+6`;
  - runtime note: все эффекты реализуются через generic `apply_status`, дополнительные ветки в `BattleEffectRuntime` не требуются.
