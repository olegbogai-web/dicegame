...
- `reroll` / «Переброс кубика» (`content/abilities/definitions/reroll.tres`):
  ...
  - ветка `1` (`reroll_upgrade_1`) — `2` использования за ход;
    ...
    - `1.2` (`reroll_upgrade_1_2`) — `2` использования за ход + лечение на значение выбранного куба;
  - ветка `2` (`reroll_upgrade_2`) — можно использовать без КД, но требуется любой куб для активации;
    - `2.1` (`reroll_upgrade_2_1`) — накладывает на владельца `regeneration` (2);
    - `2.2` (`reroll_upgrade_2_2`) — снимает `1` стак случайного отрицательного состояния с владельца;
  - runtime note: для `1.2` поддержан `healing` с `parameters.target = source`, для `2.2` добавлен generic effect type `remove_random_negative_status`;
  - редкость: `UNCOMMON`.
